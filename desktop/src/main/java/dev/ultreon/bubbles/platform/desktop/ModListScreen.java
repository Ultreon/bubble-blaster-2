...
public class ModListScreen extends Screen {
    ...
    private void renderEntry(Renderer renderer, float width, float height, float y, ModContainer entry, boolean selected, boolean hovered) {
        ...
        renderer.fill(0, y, width, height, Colors.argb(hovered ? 0x40ffffff : 0x20ffffff));
        if (selected)
        ...
        renderer.scissored(this.modList.getX(), y, width, height, () -> {
            ...
            renderer.drawText(Fonts.MONOSPACED_BOLD.get(), metadata.getId(), textX, y + 20, Colors.rgba(0xff, 0xff, 0xff, 0x80));
            renderer.drawText(Fonts.SANS_HEADER_1.get(), metadata.getName(), textX, y + 36, Colors.WHITE);
            renderer.drawText(Fonts.SANS_ITALIC.get(), metadata.getDescription(), textX, y + height - 25, Colors.rgba(0xff, 0xff, 0xff, 0x80));
        });
    }
    ...
    private class InfoContainer extends Container {
        ...
        private void drawModDetails(Renderer renderer, ModMetadata metadata, AtomicInteger textX, int textY) {
            ...
            renderer.drawText(Fonts.SANS_TITLE.get(), metadata.getName(), textX.get(), textY, Colors.WHITE);
            renderer.drawText(Fonts.MONOSPACED_HEADING_2.get(), metadata.getVersion().getFriendlyString(), textX.get() + this.layout.width, textY + Fonts.SANS_BETA_LEVEL_UP.get().getLineHeight() / 2 - Fonts.MONOSPACED_BOLD.get().getLineHeight() + 1, Colors.argb(0x80ffffff));
            renderer.drawText(Fonts.MONOSPACED_BOLD.get(), metadata.getId(), textX.get(), textY + Fonts.SANS_TITLE.get().getLineHeight() - Fonts.MONOSPACED_BOLD.get().getLineHeight(), Colors.argb(0x80ffffff));
            renderer.drawText(Fonts.MONOSPACED_BOLD.get(), metadata.getId(), textX.get(), textY + Fonts.SANS_TITLE.get().getLineHeight() - Fonts.MONOSPACED_BOLD.get().getLineHeight(), Colors.argb(0x80ffffff));
            var description = metadata.getDescription();
            var i = new AtomicInteger();
            description.lines().forEachOrdered(line -> renderer.drawText(this.font, line, textX.get(), this.y + 90 + i.getAndUpdate(this::addFontHeight) * (this.font.getLineHeight() + 1), Colors.argb(0x60ffffff)));
        }
