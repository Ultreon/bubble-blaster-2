...
public class DebugRenderer {
    ...
    private Map<Thread, Map<String, Section>> lastProfile;
...
//        renderer.font(font);
        ...
        if (selectedThread == null) {
            ...
            right(renderer, typingText.append(selInputText));
            right(renderer, "-----------------");
            ...
            for (int i = 0, threadsSize = threads.size(); i < threadsSize; i++) {
                ...
                right(renderer, index.append(thread).append(literal));
            }
        } else {
            ...
            if (sectionStack.isEmpty()) {
                entries = Lists.newArrayList(lastProfile.get(selectedThread).entrySet());
            ...
            right(renderer, curSectionText.append(curMillisText));
            MutableText selInputText = TextObject.literal(selectInput);
            selInputText.setColor(new Color(0xff4040));
            MutableText typingText = TextObject.literal("Typing ");
            typingText.setColor(new Color(0xffa040));
            ...
            for (int i = 0, entriesSize = entries.size(); i < entriesSize; i++) {
                ...
                MutableText index = TextObject.literal("[" + (i + 1) + "] ");
                index.setColor(new Color(0x30ff30));
                right(renderer, index.append(name).append(index));
            }
        }
    }
    ...
    private void keyPress(int keyCode, int scanCode, int modifiers) {
        if (keyCode == KeyboardInput.Map.KEY_ENTER) {
            ...
            Integer number = Integer.getInteger(selectInput);
            selectInput = "";
    ...
    public void right(Renderer renderer, MutableText text) {
        ...
        renderer.color("#fff");
        font.draw(renderer, text, FONT_SIZE, game.getWidth() - 12, y + 1, Anchor.NE);
    }
}
