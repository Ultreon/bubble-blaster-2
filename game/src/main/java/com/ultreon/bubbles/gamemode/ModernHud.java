...
public class ModernHud extends GameHud {
    ...
    public void renderHUD(Renderer renderer) {
        ...
        if (!gameOver) {
            drawBadge(renderer.subInstance(20, 20, 300, 80), player);
            drawStatusEffects(renderer.subInstance(game.getWidth() - 310, 20, 300, game.getHeight() - 40), player);
        }
    ...
    private void drawBadge(Renderer renderer, Player player) {
        drawBadgeBackground(renderer);
        drawPlayerDetails(renderer, player);
    }
